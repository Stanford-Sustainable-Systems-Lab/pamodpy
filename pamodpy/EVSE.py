...
class EVSE():
    def __init__(self, name, num_units=0, station=None):
        ...
        self.evse_unit_cost = df.loc[idx]['per_plug_hardware_cost'].values[0] + df.loc[idx]['per_plug_installation_cost'].values[0]   # [$]
        self.p_infra_marginal = self.evse_unit_cost #+ (84 + 240)/2 + (200 + 250)/2
        self.p_infra_capital = df.loc[idx]['other_per_site_costs'].values[0] #0.1 * (df.loc[idx]['capital_10unit_cost'].values[0] + (1500+3500)/2 + (325+1000)/2)
